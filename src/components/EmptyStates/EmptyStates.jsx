  ...
  const { t } = useTranslation();
    return (
        <div className="flex flex-col items-center justify-center py-20 px-6 text-center animate-fadeIn">
            ...
            <h3 className="text-2xl font-bold mb-3">{t('لا توجد سور مفضلة')}</h3>
            <p className="text-gray-400 mb-6 max-w-md">
                {t('ابدأ بإضافة السور المفضلة لديك بالضغط على أيقونة القلب ❤️')}
            </p>
            <div className="flex items-center gap-2 text-sm text-content-secondary">
                <span>💡</span>
                <span>{t('نصيحة: يمكنك الوصول السريع للسور المفضلة من هنا')}</span>
            </div>
        </div>
    );
}
